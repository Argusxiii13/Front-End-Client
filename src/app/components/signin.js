...
const Modal = ({ isOpen, onClose }) => {
  ...
  const handleRecover = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}api/client/forgot-password`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center ${isOpen ? 'block' : 'hidden'}`}>
      ...
      <motion.div
        ...
        className="bg-white rounded-lg shadow-lg p-6 z-10 w-96"
      >
        <h2 className="text-lg font-bold mb-4">Recover Your Password</h2>
        <input
          type="email"
        ...
        />
        {message && (
          <div className={`p-4 ${isSuccess ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'} rounded-md mb-4`}>
            {message}
          </div>
        )}
        <button
          ...
          className={`w-full py-2 rounded-md transition-all duration-200 ${isSuccess ? 'bg-green-600' : 'bg-indigo-600'} text-white ${isButtonDisabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-indigo-700'}`}
        >
          {isSuccess ? 'Email Sent!' : 'Recover'}
        </button>
      </motion.div>
    </div>
  );
            ...
            <div className="rounded-md shadow-sm -space-y-px">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  required
                  className="appearance-none rounded-b-md relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm transition-all duration-200 pr-10"
